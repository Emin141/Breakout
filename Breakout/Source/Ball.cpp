...
void Ball::Redirect(const float paddleCenterX, const float paddleSizeX) {
	...
	visually. I have also attached the file so it can be examined. I still had to 
	do some paperwork, but that is not displayed. The functions are hence
	...
	"normalized" to a range of [-1.0f, 1.0f]. (Has a bug lol)
